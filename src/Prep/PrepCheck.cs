...
public static class PrepCheck
{
    ...
    public static uint StatusIcon(uint statusId)
        => Cached(_statusIcons, statusId, id =>
            GameData.English<Status>()?.GetRowOrDefault(id) is { } row ? (uint)row.Icon : 0u,
            "prep status icon");

    private static uint ItemIcon(uint itemId)
        => Cached(_itemIcons, itemId, id =>
            GameData.English<Item>()?.GetRowOrDefault(id) is { } row ? (uint)row.Icon : 0u,
            "prep food icon");
    ...
    public static float RecastFor(Buff medicated)
    {
        ...
        return Cached(_itemRecasts, itemId, id =>
            GameData.English<Item>()?.GetRowOrDefault(id) is { } row ? row.Cooldowns : 0u,
            "prep potion recast");
    }
    ...
    public static bool IsBattleFood(Buff food)
    {
        ...
        {
            var item = GameData.English<Item>()?.GetRowOrDefault(id);
            if (item is not { } row) return Battle;
            // Food hangs its stats off ItemAction's ItemFood row.
            var act = GameData.English<ItemAction>()?.GetRowOrDefault(row.ItemAction.RowId);
            if (act is not { } a) return Battle;
            var stats = GameData.English<ItemFood>()?.GetRowOrDefault(a.Data[1]);
