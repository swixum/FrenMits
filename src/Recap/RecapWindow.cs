...
public class RecapWindow : Window
{
        ...
        List<MitRecap.MitEvent> events, List<string> party, HashSet<string> reported)
    {
        ...
        foreach (var e in events)
        {
            ...
            foreach (var pm in CooldownTracker.PlanMitsCached(e.Mit)) applied.Add(pm.Name);
        ...
        foreach (var (slot, line) in PlannedLinesNear(fight, t0, _plugin.GetActiveJobAbbr(fight)))
            foreach (var pm in CooldownTracker.PlanMitsCached(line.Action))
            {
    ...
    {
        var d = CooldownTracker.PlanInfo(e.Mit)?.Duration ?? 0f;
        return d >= 3f ? d : 15f;
    }
