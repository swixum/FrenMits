...
public static class Builtin
{
    ...
    // Where Sheets\ lives. The host points this at the plugin directory; the
    // default is right for the offline test host, where they sit beside us.
    public static string SheetsRoot { get; set; } =
    ...
    public static void WarmSheets()
        => System.Threading.Tasks.Task.Run(() =>
        {
            ...
            catch (Exception ex) { EncounterLog.Failed("sheet warm", ex); }
        });
    ...
    private static FightDefinition? LoadJsonDef(uint territory)
    {
        lock (_jsonCache)
        {
            ...
            var path = System.IO.Path.Combine(SheetsRoot, "Sheets", $"{safeName}.json");
    ...
    public static string DefaultSlotForJob(uint territory, string? jobAbbr, SlotPrefs saved = default)
    {
        var slots = Slots(territory);
        if (slots.Length == 0) return "";
        var hit = DefaultSlotForJobIn(slots, jobAbbr, saved);
        return hit.Length > 0 ? hit : slots[0];
    }

    // Same guess with no fallback, so "" means ask.
    public static string DefaultSlotForJobIn(IReadOnlyList<string> slots, string? jobAbbr, SlotPrefs saved = default)
    {
        if (slots.Count == 0 || Jobs.ByAbbreviation(jobAbbr) is not { } job) return "";

        // Apply specific job preferences first! (e.g. PCT -> D4, RDM -> M2)
        if (saved.ByJob != null && saved.ByJob.TryGetValue(job.Abbreviation, out var jobPref))
        {
            foreach (var s in slots)
        ...
        if (saved.ByRole != null && saved.ByRole.TryGetValue(job.Role, out var rolePref))
        {
            foreach (var s in slots)
